...
class TPOTBase(BaseEstimator):
    ...
    def __init__(self, generations=100, population_size=100, offspring_size=None,
                 ...
                 verbosity=0, disable_update_check=False, db=-1):
        ...
        # generations
        self.evaluated_individuals_train = {}
        self.evaluated_individuals_test = {}
        ...
        for operator in self.operators:
            ...
            for key in sorted(operator.import_hash.keys()):
                ...
                if key.startswith('tpot.'):
                    exec('from {} import {}'.format(key[14:], module_list))
                else:
                    exec('from {} import {}'.format(key, module_list))

                for var in operator.import_hash[key]:
                    self.operators_context[var] = eval(var)
        #print(self._pset.primitives[np.ndarray][0].name)
    ...
    def fit(self, features, target, test_features, test_target, sample_weight=None, groups=None):
        ...
        self._toolbox.register('evaluate', self._evaluate_individuals, features=features, target=target, test_features=features, test_target=target, sample_weight=sample_weight, groups=groups, test=False)
        self._toolbox.register('test_evaluate', self._evaluate_individuals, features=features, target=target, test_features=test_features, test_target=test_target, sample_weight=sample_weight, groups=groups, test=True)
        ...
        #self._pbar = tqdm(total=total_evals, unit='pipeline', leave=False, disable=not (self.verbosity >= 2), desc='Optimization Progress')

        try:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                pop, _, accs, lens, test_accs, param = eaMuPlusLambda(
                    population=pop,
            ...
            np.savetxt("results/TrainAccs-DB" + str(self.db) + "-Run" + str(self.random_state) + ".csv", accs)
            np.savetxt("results/Lens-DB" + str(self.db) + "-Run" + str(self.random_state) + ".csv", lens)
            np.savetxt("results/Params-DB" + str(self.db) + "-Run" + str(self.random_state) + ".csv", param)
            np.savetxt("results/TestAccs-DB" + str(self.db) + "-Run" + str(self.random_state) + ".csv", test_accs)
    ...
    def _evaluate_individuals(self, individuals, features, target, test_features, test_target, sample_weight=None, groups=None, test=True):
        ...
        for individual in unique_individuals:
            ...
            if sklearn_pipeline_str.count('PolynomialFeatures') > 1:
                ...
                if test:
                    self.evaluated_individuals_test[individual_str] = (5000., -float('inf'))
                else:
                    self.evaluated_individuals_train[individual_str] = (5000., -float('inf'))
                #if not self._pbar.disable:
                    #self._pbar.update(1)
            # Check if the individual was evaluated before
            elif (test and individual_str in self.evaluated_individuals_test) or (not test and individual_str in self.evaluated_individuals_train):
                if self.verbosity > 2:
        ...
        for resulting_score, individual_str in zip(resulting_score_list, eval_individuals_str):
            if type(resulting_score) in [float, np.float64, np.float32]:
                if test:
                    self.evaluated_individuals_test[individual_str] = (operator_counts[individual_str], resulting_score)
                else:
                    self.evaluated_individuals_train[individual_str] = (operator_counts[individual_str], resulting_score)
            else:
                raise ValueError('Scoring function does not return a float.')
        if test:
            return [self.evaluated_individuals_test[str(individual)] for individual in individuals]
        else:
            return [self.evaluated_individuals_train[str(individual)] for individual in individuals]
    @_pre_test
    ...
    def _generate(self, pset, min_, max_, condition, type_=None):
        ...
        stack = [(0, type_)]
        #for i in pset.primitives[type_]:
            #print(i.name)
        while len(stack) != 0:
