...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen, pbar,
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    fitnesses_test = toolbox.test_evaluate(invalid_ind)
    test_accs = [x[1] for x in fitnesses_test]
    accuracies = [x[1] for x in fitnesses]
    lengths = [x[0] for x in fitnesses]

    params = [len(pipe) for pipe in invalid_ind]
    ...
    for gen in range(1, ngen + 1):
        try:
            ...
            high_score = abs(max([halloffame.keys[x].wvalues[1] for x in range(len(halloffame.keys))]))
            print('Generation {0} - Current best internal CV score: {1}'.format(gen, high_score))
            ...
            pass
    return population, logbook, np.array(accuracies), np.array(lengths), np.array(test_accs), np.array(params)
    ...
    def run(self):
        ...
        try:
            with warnings.catch_warnings():
                ...
                self.args[0].fit(self.args[1], self.args[2])
                self.result = self.args[0].score(self.args[3], self.args[4])
                #print(self.result)
                #self.result = 0.5
...
def _wrapped_cross_val_score(sklearn_pipeline, features, target, test_features, test_target,
                             cv, scoring_function, sample_weight,
                             max_eval_time_mins, groups, test):
